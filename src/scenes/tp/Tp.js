...
const TC = () => {
  ...
  const routeToUpdate = (id,n) =>{
    ...
    history(`/updatetypecourrier/${id}/${n}`)  
}
  const deleteById = async (id) =>{
    const q = query(collection(firestore, "typecourrier"), where("id", "==", id));
    const table = []
    ...
    if (answer) {
        await deleteDoc(doc(firestore,"typecourrier",table[0]))
  ...
  return (
    <Box m="20px">
      <Header button = {"Ajouter"} buttonLink = "/createtypecourrier" title="Types de courriers" subtitle="chaque courrier doit corresponde à un type" />
      <Box
        m="40px auto"
        height="70vh"
        width ="75vw"
