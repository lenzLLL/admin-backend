...
const CreateTc = () => {
  ...
  const saveDetails = async (name) =>{
    try{
                ...
                try{
                    ...
                    // }
                    setIsLoading(true)
                    await setDoc(doc(firestore,"typecourrier",`${Date.now()}`),data,{merge:true}).then(
                      ()=>{
                        alert("type courrier ajouté")
  ...
  return (
    <Box m="20px">
      <Header title="Type de courrier" subtitle="Ajouter un nouveau type de courrier" />
