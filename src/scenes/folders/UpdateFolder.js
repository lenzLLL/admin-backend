...
const UpdateFolder = () => {
  ...
  const getPoste = async () =>{
    const q = query(collection(firestore, "folders"), where("id", "==", JSON.parse(localStorage.getItem("ASYNC_ID"))));
    const table = []
  ...
  const handleSubmit =  async (e) => {
    ...
    if(!name)
    {
      ...
      const ref = doc(firestore,"folders",u)
  ...
  return (
    <Box m="20px">
      <Header title="Dossiers" subtitle="Modifier le dossier" />
