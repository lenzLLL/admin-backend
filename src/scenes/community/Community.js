...
import {community as D} from "../../data/Personnel"
...
const Community = ({onSet}) => {
  ...
  const columns = [
    { field: "id", headerName: "ID" },
    {
        ...
        renderCell: ({ row: { avatar } }) => {
          return (
              <div>
                  <img src = {avatar} style = {{borderRadius:"50%",width:"40px",height:"40px"}}/>
              </div>
          )}
  ...
  return (
    <Box m="20px">
      <Header  title="Communauté" subtitle="Retrouver toute la communauté" />
      <Box
        m="40px 0 0 0"
        height="70vh"
        sx={{
      ...
      >
        <DataGrid checkboxSelection rows={U} columns={columns} />
      </Box>
    </Box>
  );
};
